...
<header class="header">
    ...
    <form action="fresh.php" class="search-form" method="get">
...
<section class="product" id="products">
    <div class="item-list">
        ...
        $sql = "SELECT * FROM products WHERE product_id BETWEEN 3000 AND 3999";
        ...
        if ($result && $result->num_rows > 0) {
            while ($product = $result->fetch_assoc()) {
                ...
                // Category Filtering 
                $category = 'other';
                if (str_contains($productLower, 'apple') ||
                    str_contains($productLower, 'banana') ||
                    str_contains($productLower, 'peach') ||
                    str_contains($productLower, 'grape') ||
                    str_contains($productLower, 'navel orange')
                ) $category = 'fruits';
                if (str_contains($productLower, 't bone steak')) {
                    $category = 'steak';
                }
                if (str_contains($productLower, 'cheddar cheese')) {
                    $category = 'cheese';
                }
        ...
        if (!empty($matched)) {
            ...
            echo "<p style='text-align:center;'>No Fresh products available</p>";
        }
        ?>
    </div>
</section>
