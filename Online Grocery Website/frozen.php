...
<header class="header">
    ...
    <form action="frozen.php" class="search-form" method="get">
...
<section class="product" id="products">
    <div class="item-list">
        ...
        $sql = "SELECT * FROM products WHERE product_id BETWEEN 1000 AND 1005";
        ...
        if ($result && $result->num_rows > 0) {
            while ($product = $result->fetch_assoc()) {
                ...
                // Category Filtering 
                $category = 'others';
                if (str_contains($productLower, 'tub ice cream')) $category = 'desserts';
                if (
                    str_contains($productLower, 'fish fingers') ||
                    str_contains($productLower, 'hamburger patties') ||
                    str_contains($productLower, 'shelled prawns')
                ) $category = 'meals';

                $productLower = strtolower($productName);
                $unitQtyLower = strtolower($unitQty);
                
                // Applying search and filter logic
                if (!empty($search)) {
                    if (!str_contains($productLower, $search) && !str_contains($unitQtyLower, $search)) {
                        continue;
                    }
                }
        ...
        if (!empty($matched)) {
            foreach ($matched as $product) {
                ...
                echo "
                <div class='item-box' data-category='{$category}' data-name='" . strtolower($productName) . "'>
                    <div class='image-box'>
            ...
            echo "<p style='text-align:center;'>No Frozen products available.</p>";
        }
        ?>
    </div>
</section>
