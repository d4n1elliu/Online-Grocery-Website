...
<header class="header">
    ...
    <form action="homeuse.php" class="search-form" method="get">
...
<section class="product" id="products">
    <div class="item-list">
        ...
        $sql = "SELECT * FROM products WHERE product_id BETWEEN 2000 AND 2006";
        ...
        if ($result && $result->num_rows > 0) {
            while ($product = $result->fetch_assoc()) {
                ...
                // Category Filtering 
                $category = 'other';
                if (str_contains($productLower, 'bath soap') ||
                    str_contains($productLower, 'garbage bags') ||
                    str_contains($productLower, 'washing powder') ||
                    str_contains($productLower, 'laundry bleach')
                ) {
                    $category = 'cleaning';
                }
                if (str_contains($productLower, 'panadol')) {
                    $category = 'health';
                }
        ...
        if (!empty($matched)) {
            ...
            echo '<p style="text-align:center;">No HomeUse products available.</p>';
        }
        ?>
    </div>
</section>
